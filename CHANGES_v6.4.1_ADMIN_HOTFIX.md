...
## v6.4.1 — 배포 후 발견된 크래시 수정 (`ERR_REQUIRE_ESM`)

v6.4.0을 실제 Vercel에 배포해보니 `/api/admin-list-users` 등 관리자 API가 아래 오류로 크래시되는 문제가 발견되어 수정했습니다.

```
Error [ERR_REQUIRE_ESM]: require() of ES Module /var/task/node_modules/jose/dist/webapi/index.js
```

**원인**: `firebase-admin`이 내부적으로 사용하는 `jwks-rsa` 패키지가 `jose` 라이브러리를 불러오는데, `jose` 최신 버전(6.x)은 ESM 전용(CommonJS `require()` 진입점이 없음)이라 Vercel 서버 함수 실행 환경과 충돌하며 함수 자체가 시작하지 못하고 죽어버립니다. (관리자 인증 로직과는 무관한, 순수 패키지 버전 호환성 문제입니다.)

**수정**: `package.json`에 아래 `overrides`를 추가해 `jose`를 CommonJS 진입점이 있는 5.9.6 버전으로 고정했습니다.
...
이 변경 후 `npm install`로 `package-lock.json`을 재생성했고, `npm run build` / `npx oxlint` 모두 다시 정상 통과를 확인했습니다.

**적용 방법**: 이 zip의 `package.json`과 `package-lock.json` 두 파일을 GitHub 저장소의 기존 파일에 덮어써서 커밋 → Vercel이 자동으로 재배포합니다. (다른 소스 파일은 v6.4.0과 동일합니다.)
