...
export default function App() {
  ...
  const completeFinishRound=async(target?:Round)=>{const source=target??activeRound;if(!source)return;setShowFinishWarning(false);const completed:Round={...source,status:'completed',finished:true};setActiveRound(completed);setScreen('summary');setSaveStatus('saving');try{await saveHoles(user.uid,completed.id,completed.holes,completed.parkPlayers,completed.holeCount);await finishRound(user.uid,completed.id);removeDraft(completed.id);flashSaved()}catch(err){console.error(err);saveDraft(completed);setSaveStatus('error');setShowSaveWarning(true)}};
  // 전반 9홀까지만 치고 후반은 그만두고 싶을 때: holes를 앞 9개로 줄이고 holeCount도 9로 맞춘 뒤 곧바로 종료 처리한다.
  const finishAtFrontNine=()=>{if(!activeRound)return;setShowFrontNine(false);const holes=activeRound.holes.slice(0,9);const truncated:Round={...activeRound,holes,holeCount:9,...calcTotals(holes)};setActiveRound(truncated);saveDraft(truncated);if(holes.some(h=>h.score<=0)){setShowFinishWarning(true);return;}void completeFinishRound(truncated)};
  // 9홀 라운드를 다 마친 뒤 18홀로 이어가고 싶을 때: 10~18번 홀 9개를 기본 Par4로 추가하고 holeCount를 18로 바꾼다.
  const extendToEighteen=()=>{if(!activeRound||activeRound.holeCount!==9)return;const extra:Hole[]=Array.from({length:9},(_,i)=>({number:10+i,par:4,score:0,putts:0,fairway:'na' as const,gir:null,penalty:0}));const holes=[...activeRound.holes,...extra];const extended:Round={...activeRound,holes,holeCount:18,...calcTotals(holes),updatedAt:Date.now()};setActiveRound(extended);saveDraft(extended);void saveHoles(user.uid,extended.id,holes,extended.parkPlayers,18).catch(console.error);setHoleIndex(9)};
  ...
  return <div className="app-shell">
    ...
    {showFrontNine&&<div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/45 p-5"><div className="w-full max-w-sm rounded-3xl bg-white p-6 text-center shadow-2xl"><div className="text-4xl">🎉</div><h2 className="mt-3 text-xl font-bold">전반 9홀이 끝났습니다!</h2><p className="mt-2 text-sm text-gray-500">수고하셨어요. 잠시 쉬었다가 후반 라운드를 이어가세요.</p><button onClick={continueBackNine} className="mt-6 h-13 w-full rounded-2xl bg-brand px-5 py-4 font-bold text-white">후반 10홀 시작하기</button><button onClick={finishAtFrontNine} className="mt-2 h-12 w-full rounded-2xl bg-gray-100 font-semibold text-gray-600">여기서 종료 (9홀로 마감)</button></div></div>}
