...
import type { Hole, Round, SportType } from './types';
...
export default function App() {
  ...
  const persistActiveRound=async(holes:Hole[])=>{if(!activeRound)return;setSaveStatus('saving');try{await saveHoles(user.uid,activeRound.id,holes);localStorage.removeItem(`golf-scorecard-draft-${activeRound.id}`);setShowSaveWarning(false);flashSaved()}catch(err){console.error(err);localStorage.setItem(`golf-scorecard-draft-${activeRound.id}`,JSON.stringify({...activeRound,holes,updatedAt:Date.now()}));setSaveStatus('error');setShowSaveWarning(true)}};
