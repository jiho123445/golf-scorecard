...
export function NewParkRound({ ownerName, onBack, onStart }: Props) {
  const [date, setDate] = useState(todayString());
  const [courseName, setCourseName] = useState('');
  const [holeCount, setHoleCount] = useState<HoleCount>(18);
  const [playerCount, setPlayerCount] = useState(1);
  const [playerNames, setPlayerNames] = useState([ownerName || '김지호', '', '', '']);
  ...
  const start = async () => {
    ...
    try {
      const parkPlayers: ParkPlayer[] = Array.from({ length: playerCount }, (_, i) => ({
        id: `player-${i + 1}`,
        name: playerNames[i].trim() || (i === 0 ? (ownerName || '김지호') : `플레이어 ${i + 1}`),
  ...
  return <div className="flex flex-1 flex-col">
    ...
    <div className="flex flex-1 flex-col gap-5 overflow-y-auto px-5 py-5">
      <div className="park-photo-card rounded-3xl p-5"><p className="text-3xl">🏌️</p><h2 className="mt-3 text-lg font-bold text-emerald-950">함께 즐기는 파크골프</h2><p className="mt-1 text-sm leading-6 text-emerald-800/70">최대 4명까지 한 라운드의 홀별 타수를 함께 기록할 수 있습니다.</p></div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-4"><p className="mb-3 text-sm font-bold text-gray-700">플레이어 이름</p><div className="flex flex-col gap-2">{Array.from({ length: playerCount }, (_, i) => <label key={i} className="flex items-center gap-3"><span className="w-7 text-sm font-bold text-emerald-700">{i + 1}</span><input value={playerNames[i]} onChange={e => setName(i, e.target.value)} placeholder={i === 0 ? ownerName : `플레이어 ${i + 1}`} className="h-11 flex-1 rounded-xl border border-gray-200 px-3 text-sm" /></label>)}</div></div>
    </div>
