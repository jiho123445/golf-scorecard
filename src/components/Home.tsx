...
interface Props{displayName:string;golfStats:RoundStats;parkStats:RoundStats;recentGolf:Round[];recentPark:Round[];inProgressRounds:Round[];onStart:()=>void;onResume:(r:Round)=>void;onOpen:(r:Round)=>void;onDeleteInProgress?:(r:Round)=>void;onViewStats?:()=>void;onViewRecords?:()=>void;}
const score=(n:number|null)=>n===null?'-':Number.isInteger(n)?String(n):n.toFixed(1);
export function Home({displayName,golfStats,parkStats,recentGolf,recentPark,inProgressRounds,onStart,onResume,onOpen,onDeleteInProgress,onViewStats,onViewRecords}:Props){
 ...
 return <div className="home-screen flex-1 overflow-y-auto pb-28">
   ...
   <section className="glass-card score-overview"><div className="section-head"><h2>▣ 나의 스코어 현황</h2><button type="button" className="section-link" onClick={onViewStats}>전체보기 ›</button></div><div className="stats-grid"><Metric icon="◉" label="평균타수" value={score(golfStats.average??parkStats.average)} /><Metric icon="▥" label="최근 5경기" value={score(golfStats.last5Average??parkStats.last5Average)} /><Metric icon="★" label="베스트 스코어" value={score(golfStats.best??parkStats.best)} /><Metric icon="⚑" label="총 라운드" value={String(rounds)} /></div></section>
   <section className="glass-card recent-section"><div className="section-head"><h2>▣ 최근 기록</h2><button type="button" className="section-link" onClick={onViewRecords}>전체보기 ›</button></div>{recent.length===0?<p className="empty-record">아직 완료된 기록이 없습니다.</p>:recent.map(r=><button key={r.id} onClick={()=>onOpen(r)} className="recent-row"><div className="recent-thumb">{r.sportType==='park'?'🏌️':'⛳'}</div><div className="flex-1 text-left"><b>{r.courseName}</b><small>{formatDate(r.date)} · {r.sportType==='park'?'파크골프':'골프'}</small></div><strong>{r.totalScore}타</strong><em>›</em></button>)}</section>
