...
interface Props{displayName:string;golfStats:RoundStats;parkStats:RoundStats;recentGolf:Round[];recentPark:Round[];inProgressRounds:Round[];onStart:()=>void;onResume:(r:Round)=>void;onOpen:(r:Round)=>void;onDeleteInProgress?:(r:Round)=>void;onViewRecords?:()=>void;}
const score=(n:number|null)=>n===null?'-':Number.isInteger(n)?String(n):n.toFixed(1);
export function Home({displayName,golfStats,parkStats,recentGolf,recentPark,inProgressRounds,onStart,onResume,onOpen,onDeleteInProgress,onViewRecords}:Props){
