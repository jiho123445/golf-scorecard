...
interface Props{displayName:string;golfStats:RoundStats;parkStats:RoundStats;recentGolf:Round[];recentPark:Round[];inProgressRounds:Round[];onStart:()=>void;onResume:(r:Round)=>void;onOpen:(r:Round)=>void;}
const score=(n:number|null)=>n===null?'-':Number.isInteger(n)?String(n):n.toFixed(1);
export function Home({displayName,golfStats,parkStats,recentGolf,recentPark,inProgressRounds,onStart,onResume,onOpen}:Props){
 ...
 return <div className="home-screen flex-1 overflow-y-auto pb-28">
   ...
   {inProgressRounds.length>0&&<section className="glass-card active-round-card"><div className="section-head"><h2>▶ 진행 중인 라운드</h2><span>{inProgressRounds.length}건</span></div>{inProgressRounds.slice(0,2).map(r=>{const entered=r.holes.filter(h=>h.score>0).length;return <div className="active-round-row" key={r.id}><div className={`round-thumb ${r.sportType==='park'?'park':''}`}>{r.sportType==='park'?'🏌️':'⛳'}</div><div className="min-w-0 flex-1"><b>{r.courseName}</b><small>{r.sportType==='park'?'파크골프':'골프'} · {r.holeCount}홀 · {formatDate(r.date)}</small></div><strong>{entered} / {r.holeCount}홀</strong><button onClick={()=>onResume(r)}>이어하기 ›</button></div>})}</section>}
